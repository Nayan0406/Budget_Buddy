  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        notes: form.notes,
        reminder: { enabled: !!form.reminderEnabled, daysBefore: Number(form.reminderDays) }
      }
      const res = await fetch(`${API_BASE}/borrowings`, {
        method: 'POST',
        headers: {
      ...
      setItems(prev => [created, ...prev])
      setShowForm(false)
      setForm({ type: 'borrowed', name: '', contact: '', amount: '', dueDate: '', notes: '', reminderEnabled: false, reminderDays: 3 })
      setError('')
    } catch (err) {
      console.error(err)
      alert('Error creating record. Check console.')
        ...
        <div className="py-4 sm:py-6 px-3 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <div className="bg-indigo-100 text-indigo-600 rounded-full p-2 sm:p-3 shrink-0">
          ...
          {/* Filter + Add */}
          <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row gap-3 sm:gap-2 items-start sm:items-center justify-between">
            <div className="flex flex-wrap gap-2 w-full sm:w-auto">
              <button key="all" className="px-3 py-2 rounded-lg text-sm font-medium transition-colors bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 flex-1 sm:flex-none min-w-0">All</button>
              <button key="owed-by-me" className="px-3 py-2 rounded-lg text-sm font-medium transition-colors bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 flex-1 sm:flex-none min-w-0">Owed By Me</button>
              <button key="owed-to-me" className="px-3 py-2 rounded-lg text-sm font-medium transition-colors bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 flex-1 sm:flex-none min-w-0">Owed To Me</button>
              <button key="overdue" className="px-3 py-2 rounded-lg text-sm font-medium transition-colors bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 flex-1 sm:flex-none min-w-0">Overdue</button>
            </div>
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <button
                onClick={() => setShowForm(true)}
                className="flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 w-full sm:w-auto"
              >
          ...
          {loading ? (
            ...
            <div className="bg-white overflow-hidden shadow rounded-lg p-6 sm:p-8 text-center mx-2 sm:mx-0">
              ...
              <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">No borrowings yet</h3>
              <p className="text-sm sm:text-base text-gray-600 mb-4">Create a record to start tracking money borrowed or lent.</p>
              <button
                onClick={() => setShowForm(true)}
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-2 sm:px-3 lg:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-32 sm:min-w-40">Person</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {items.map((it, index) => (
                        <tr key={it._id || it.id || index} className="hover:bg-gray-50">
                          <td className="px-2 sm:px-3 lg:px-6 py-2 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-900">
                            <span className={`inline-flex px-1.5 py-0.5 text-xs font-medium rounded-full ${
                              ...
                              {it.type === 'borrowed' ? 'Lender' : 'Borrower'}
                            </span>
                          </td>
                          ...
                          <td className="px-2 sm:px-3 lg:px-6 py-2 sm:py-4 whitespace-nowrap text-center text-xs sm:text-sm">
                            <div className="flex justify-center gap-1 sm:gap-2">
                              <button className="text-indigo-600 hover:underline text-xs sm:text-sm" onClick={() => alert('Not implemented yet')}>Details</button>
                              <button className="text-red-600 hover:underline text-xs sm:text-sm" onClick={() => alert('Delete not implemented')}>Delete</button>
                            </div>
                          </td>
                        </tr>
                      ))}
          ...
          {items.length > 0 && (
            <div className="mt-4 sm:mt-6 bg-white overflow-hidden shadow rounded-lg p-4 sm:p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-2">
                ...
                <div key="total" className="text-center p-3 sm:p-0 bg-gray-50 sm:bg-transparent rounded-lg sm:rounded-none sm:col-span-1">
